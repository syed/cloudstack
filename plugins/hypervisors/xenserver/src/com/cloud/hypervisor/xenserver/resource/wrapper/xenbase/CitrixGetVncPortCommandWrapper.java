...
public final class CitrixGetVncPortCommandWrapper extends CommandWrapper<GetVncPortCommand, Answer, XenServerResourceBase> {

    private static final Logger s_logger = Logger.getLogger(CitrixGetVncPortCommandWrapper.class);

    @Override
    public Answer execute(final GetVncPortCommand command, final XenServerResourceBase xenServerResourceBase) {
        final Connection conn = xenServerResourceBase.getConnection();
        try {
            final Set<VM> vms = VM.getByNameLabel(conn, command.getName());
            if (vms.size() == 1) {
                String consoleurl;
                consoleurl = "consoleurl=" + xenServerResourceBase.getVncUrl(conn, vms.iterator().next()) + "&" + "sessionref=" + conn.getSessionReference();
